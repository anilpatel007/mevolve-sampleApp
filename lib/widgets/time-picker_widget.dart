...
class _TimePickerWidgetState extends State<TimePickerWidget> {
  ...
  void initState() {
    ...
    WidgetsBinding.instance?.addPostFrameCallback((timeStamp) {
      // hourController.detach(hourController.position);
    });
  ...
  Widget build(BuildContext context) {
    ...
    List<Widget> contents = [
      Padding(
        ...
        child: SizedBox(
          ...
          child: spinner(hourController, _getHourCount(),
              currentSelectedHourIndex, isHourScrolling, 1, (index) {
            currentSelectedHourIndex = index;
            isHourScrolling = true;
          }, () => isHourScrolling = false, isEnable: widget.isEnable),
        ),
      ),
      // spacer(),
      Container(color: Colors.grey.withOpacity(0.5), height: 125, width: 1,),
      ...
      Padding(
        ...
        child: SizedBox(
          ...
          }, () => isMinuteScrolling = false, isEnable: widget.isEnable),
        ),
      ),
    ...
    if (widget.isShowSeconds) {
      ...
      contents.add(SizedBox(
        ...
        }, () => isSecondsScrolling = false, isEnable: widget.isEnable),
      ));
    }
    ...
    if (!widget.is24HourMode) {
      ...
      contents.add(SizedBox(
        ...
        height: _getItemHeight()! * 3,
        child: apSpinner(isEnable: widget.isEnable),
      ));
    }
  ...
  Widget apSpinner({bool isEnable = true}) {
    Widget _spinner = NotificationListener<ScrollNotification>(
      onNotification: (scrollNotification) {
        if (scrollNotification is UserScrollNotification) {
          if (scrollNotification.direction.toString() ==
